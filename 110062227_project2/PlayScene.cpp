...
void PlayScene::Update(float deltaTime) {
    ...
    // Win
    bool trap_only = true;
    for (auto i : DefenseGroup->GetObjects()) {
        Defense*tmp = dynamic_cast<Defense*>(i);
        if (tmp->id != 4)trap_only = false;
    }
    if (DefenseGroup->GetObjects().empty()||trap_only) {
        trap_only = false;
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
    ...
    if (button & 1) {
        if (!CheckOccupied(x, y)||preview->id==3) {
            ...
            int remainId = armyAmount[preview->id] > 0 ? preview->id : -1;//lws
            // Remove Preview.
            ...
            if (preview->id == 3) {
                ...
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2 + BlockSize, y * BlockSize + BlockSize / 2 + BlockSize));
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2 + BlockSize, y * BlockSize + BlockSize / 2));
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2 + BlockSize, y * BlockSize + BlockSize / 2 - BlockSize));
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2, y * BlockSize + BlockSize / 2 + BlockSize));
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2, y * BlockSize + BlockSize / 2));
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2, y * BlockSize + BlockSize / 2 - BlockSize));
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2 - BlockSize, y * BlockSize + BlockSize / 2 + BlockSize));
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2 - BlockSize, y * BlockSize + BlockSize / 2));
                GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/snow.png", 2, x * BlockSize + BlockSize / 2 - BlockSize, y * BlockSize + BlockSize / 2 - BlockSize));
            ...
            if (remainId != -1) {
                if (remainId == 0)
                    ...
                    preview = new BombArmy(0, 0);//lws
                else if (remainId == 2)
                    preview = new ArmySolid(0, 0);//lws
                else if (remainId == 3) {
                    preview = new Ice(0, 0);//lws
                }
                preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
                preview->Tint = al_map_rgba(255, 255, 255, 200);
                if(remainId!=3)preview->Enabled = false;
...
void PlayScene::OnKeyDown(int keyCode) {
    ...
    if (keyCode == ALLEGRO_KEY_TAB) {
        ...
        for (auto c : keyStrokes) {
            if (c != code[i]) break;
            if (i == code.size() - 1) EffectGroup->AddNewObject(new Plane());
            i++;
        }
...
void PlayScene::ReadMap() {
    ...
    while (fin >> c) {
        ...
        case '5':mapData.push_back(TILE_STOP); break;
        case '\n':
        case '\r':
    ...
    for (int i = 0; i < MapHeight; i++) {
        for (int j = 0; j < MapWidth; j++) {
            ...
            case TILE_WALL:
                ...
                break;
            case TILE_CANNON: //lws
                DefenseGroup->AddNewObject(new CannonDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                //DefenseGroup->AddNewObject(new SlowDefense(j * BlockSize + BlockSize * 2, i * BlockSize + BlockSize * 1));
                break;
            case TILE_FLOOR:
                if (j <= MapWidth - 2 && j >= 2) {
                ...
                break;
            case TILE_SLOWCANNON: //lws
                //DefenseGroup->AddNewObject(new CannonDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                DefenseGroup->AddNewObject(new SlowDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                break;
            case TILE_TRAP: //lws
            //DefenseGroup->AddNewObject(new CannonDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
            ...
            case TILE_STOP: //lws
            //DefenseGroup->AddNewObject(new CannonDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                DefenseGroup->AddNewObject(new Stop(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                break;
            default:
                throw std::ios_base::failure("Map data is corrupted.");
            }
...
void PlayScene::ConstructButton(int id, std::string imageName) {
    if (id == 3) {
        ...
        // Button Label
        AddNewObject(UIArmyAmount[id] = new Engine::Label("x" + std::to_string(armyAmount[id]), "pirulen.ttf", 20.5, 230 + 120 * (id+7), BlockSize * MapHeight + 110, 0, 0, 0, 255, 0.5, 0.5));
    }
    else {
        ArmyButton* btn;
        // Button
